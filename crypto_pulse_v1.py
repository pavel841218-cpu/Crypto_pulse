        ...
        async with session.get(url, headers=headers, timeout=10) as resp:
            ...
            for item in data.get("data", []):
                ...
                try:
                    ...
                    # Извлекаем ОИ напрямую из тикера без сотни отдельный запросов
                    open_interest = float(
                ...
                result.append((symbol, price, quote_volume, open_interest))
...
async def collect_oi_for_all(session, tickers):
    ...
    for item in tickers:
        symbol = item[0]
        oi = item[3] if len(item) > 3 else 0
...
async def process_tickers(session, tickers):
    ...
    for (symbol, current_price, quote_volume, *_) in tickers:
