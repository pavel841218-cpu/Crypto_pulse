...
# ============================================================
#       CEX FUTURES AGGREGATOR v3.4 (RATE-LIMIT SAFE)
...
CANDLE_REFRESH_SEC = 45        # Увеличено до 45с для снижения нагрузки на Binance API
OI_REFRESH_SEC = 60
...
# ------------------------------------------------------------
# BITGET SCANNER (первичный фильтр)
# ------------------------------------------------------------
...
BINANCE_MIN_LEAD_PCT = -0.2    # Binance не должен отставать
BINANCE_MAX_LEAD_PCT = 3.0     # Binance не должен уходить слишком далеко
...
async def http_get(url, params=None, headers=None, timeout=10, exchange="other"):
    ...
    try:
        ...
        async with SESSION.get(
            ...
            if resp.status == 429:
                log.warning("429 rate limit: %s. Pacing...", exchange)
...
# ============================================================
# BINANCE FUTURES (С ЗАЩИТОЙ ОТ БАНА)
# ============================================================
...
async def fetch_binance_candles(symbol):
    # Микро-пауза для предотвращения блокировки IP по Rate Limit
...
# ============================================================
# PROCESS SYMBOL — STRICT FILTERING
# ============================================================
...
async def process_symbol(base):
    ...
    # 1. Сканируем Bitget (первичный фильтр)
    ...
    if bg["body_ratio"] < BITGET_MIN_BODY_RATIO or bg["close_position"] < BITGET_MIN_CLOSE_POSITION:
        STATS["rejected_bitget_structure"] += 1
        return

    # Bitget прошёл проверку — найдена аномалия
    STATS["bitget_candidates"] += 1

    # 2. Запрашиваем Binance ТОЛЬКО при наличии прострела на Bitget
    ...
    if lead_pct < BINANCE_MIN_LEAD_PCT or lead_pct > BINANCE_MAX_LEAD_PCT:
...
async def refresh_universe():
    ...
    if not bitget or not binance_futures:
        log.warning("Universe refresh incomplete")
        return
    ...
    log.info(
        "UNIVERSE | Bitget=%d | BN Futures=%d | In range: %d | Common: %d",
        len(bitget), len(binance_futures), in_range, len(COMMON_SYMBOLS)
    )
...
async def scan_cycle():
    ...
    candidates = list(COMMON_SYMBOLS.keys())[:MAX_CANDLE_CANDIDATES]
    STATS["bitget_candidates"] = 0
    
    # Семафор снижен до 2 для устранения сетевых пиков к Binance
...
async def hourly_diagnostics():
    ...
    msg = (
        "🩺 <b>ЧАСОВАЯ ДИАГНОСТИКА v3.4 (SAFE-RATE)</b>\n\n"
        f"Common символов: <b>{len(COMMON_SYMBOLS)}</b>\n"
...
def stats_text():
    return f"""
CEX FUTURES AGGREGATOR v3.4 (SAFE-RATE)
=====================================
...
async def start_background(app):
    ...
    startup_msg = (
        "🚀 <b>CEX FUTURES AGGREGATOR v3.4 ЗАПУЩЕН</b>\n\n"
        "• Фильтр Rate-Limit обновлен (Binance оптимизирован)\n"
        "• База RVOL: <b>24 часа (288 свечей)</b>\n"
        "• Мониторинг рынка возобновлен."
    )
