...
# ============================================================
#         РАСЧЕТ RVOL (ОТНОСИТЕЛЬНОГО ОБЪЕМА)
# ============================================================

async def fetch_rvol(session, symbol, sem):
    """Считает отношение объема текущей свечи к среднему объему за 20 предыдущих свечей."""
    async with sem:
...
async def process_tickers(session, tickers):
    ...
    for symbol, current_price, quote_volume in tickers:
        ...
        logging.info("🚀 СИГНАЛ | %s | %s%.2f%% | RVOL: %.2fx", clean_symbol, sign, change_pct, rvol)
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        logging.info("🚀 CONSOLIDATION HUNTER С НАБЛЮДЕНИЕМ RVOL ЗАПУЩЕН")
