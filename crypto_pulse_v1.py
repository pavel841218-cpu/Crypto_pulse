...
# Фильтры накопительной базы и Live-пробоя
MIN_24H_VOLUME_USDT = 30_000      # Снижено до $30k для ловли затаившихся альтов
MAX_BASE_RANGE_PCT = 18.0         # Допуск ширины полки до 18%
ACCUMULATION_CANDLES_15M = 8      # База от 2 часов на 15M (для быстрых пампов)
ACCUMULATION_CANDLES_4H = 4       # База от 16 часов на 4H

# ПОРОГИ ДЛЯ ТЕКУЩЕЙ (ОТКРЫТОЙ) СВЕЧИ
MIN_BREAKOUT_PCT = 3.5            # Ловим вход от +3.5%
MAX_BREAKOUT_PCT = 15.0           # Расширено до +15.0% для импульсных свечей
MIN_RVOL = 0.8                    # Минимальный прогнозируемый RVOL
MAX_RSI = 88.0                    # Допуск по RSI

CHECK_INTERVAL_SECONDS = 30       # Сканирование каждые 30 секунд
ALERT_COOLDOWN_SECONDS = 7200     # Кулдаун на одну монету — 2 часа

EXCLUDE_KEYWORDS = [
    "XAUT", "GOLD", "PAXG", "USDC", "ETHDAI",
...
@app.route("/")
def home():
    return "MacroPulse Live | Active", 200
...
def is_midnight_window() -> bool:
    """Проверка смены суток UTC (время сброса фандинга и сухих пампов)."""
    now = datetime.now(timezone.utc)
    return (now.hour == 23 and now.minute >= 50) or (now.hour == 0 and now.minute <= 20)
    ...
    try:
        async with session.get(url, timeout=10) as resp:
            ...
            for item in data.get("data", []):
                ...
                # Порог объема снижается во время смены суток (Midnight Pump)
                effective_min_vol = MIN_24H_VOLUME_USDT * (0.5 if is_midnight_window() else 1.0)
                if vol >= effective_min_vol and price > 0:
...
async def analyze_setup(session, symbol, current_price, interval="15m", accum_candles=8):
    url = "https://open-api.bingx.com/openApi/swap/v3/quote/klines"
    symbol_api = to_api_format(symbol)
    params = {"symbol": symbol_api, "interval": interval, "limit": 40}
    try:
        async with session.get(url, params=params, timeout=6) as resp:
            ...
            if len(closed_candles) < accum_candles + 3:
                return None

            base_candles = closed_candles[-accum_candles:]
            ...
            elapsed_sec = max(time.time() - candle_open_sec, 30.0)
            
            candle_duration = 900.0 if interval == "15m" else 14400.0
            projected_vol = (current_vol / elapsed_sec) * candle_duration
            ...
            return {
                ...
                "base_high": max_p,
                "base_low": min_p,
                "duration_hours": (accum_candles * 0.25) if interval == "15m" else (accum_candles * 4)
            }
    except Exception:
        return None
...
async def process_ticker(session, ticker, semaphore):
    ...
    async with semaphore:
        ...
        # 1. Сначала проверяем быстрый 15M таймфрейм
        setup = await analyze_setup(session, symbol, price, interval="15m", accum_candles=ACCUMULATION_CANDLES_15M)
        # 2. Если на 15M пробоя нет, проверяем макро-структуру 4H
        if not setup:
            setup = await analyze_setup(session, symbol, price, interval="4h", accum_candles=ACCUMULATION_CANDLES_4H)

    if setup:
        sent_alerts[symbol] = now
        coin = symbol.split("-")[0]
        mid_tag = "🌙 MIDNIGHT " if is_midnight_window() else ""
        
        msg = (
            f"💣 <b>{mid_tag}ИМПУЛЬС ИЗ ПОЛКИ ({setup['interval'].upper()})</b> | <code>{coin}</code>\n\n"
            f"📦 <b>База:</b> {setup['duration_hours']:.1f}ч в диапазоне {setup['range_pct']:.2f}%\n"
            f"🚀 <b>Пробой live-свечи:</b> +{setup['breakout_pct']:.2f}%\n"
            f"📊 <b>Прогноз RVOL:</b> {setup['rvol']:.2f}x\n"
            f"📈 <b>RSI:</b> {setup['rsi']:.1f}\n\n"
            f"🎯 <b>Верх полки:</b> {format_price(setup['base_high'])}\n"
            f"🛡 <b>Низ полки (Стоп):</b> {format_price(setup['base_low'])}\n"
...
async def main():
    async with aiohttp.ClientSession() as session:
        logging.info("MACROPULSE (15M / 4H Live Pump Hunter) Запущен")
        asyncio.create_task(self_ping())

        await asyncio.sleep(2)

        startup_msg = (
            "🚀 <b>БОТ MACROPULSE УСПЕШНО ОБНОВЛЕН И ЗАПУЩЕН!</b>\n\n"
            "⚙️ <b>Параметры отслеживания:</b>\n"
            f"• Таймфреймы: 15M (быстрый) / 4H (макро)\n"
            f"• Пробой live-свечи: +{MIN_BREAKOUT_PCT}% ... +{MAX_BREAKOUT_PCT}%\n"
            f"• Мин. суточный объем: ${MIN_24H_VOLUME_USDT:,.0f}\n"
