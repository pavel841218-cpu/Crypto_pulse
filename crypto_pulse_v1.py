...
# Черный список (без USDT, чтобы не отсекать все фьючерсы)
EXCLUDE_KEYWORDS = [
...
# ============================================================
#                 ФОРМАТИРОВАНИЕ ТИКЕРОВ
# ============================================================

def to_ticker_format(symbol: str) -> str:
    """Приводит символ к виду BTC-USDT (с дефисом)."""
...
def to_api_format(symbol: str) -> str:
    """Приводит символ к виду BTCUSDT (без дефиса)."""
...
def is_crypto_symbol(symbol: str) -> bool:
    """Проверяет, является ли символ криптовалютой (без нефти, золота, индексов)."""
    ...
    try:
        async with session.get(url, timeout=10) as resp:
            ...
            for item in data.get("data", []):
                ...
                sym = to_ticker_format(raw_sym)  # приводим к BTC-USDT
        ...
        logging.error(f"Exception in get_top_tickers: {e}")
        return []

async def analyze_4h_setup(session, symbol, current_price):
    url = "https://open-api.bingx.com/openApi/swap/v3/quote/klines"
    symbol_api = to_api_format(symbol)  # BTCUSDT для klines
