...
# Снижено с 400 до 150 — защита от 429 Bitget
MAX_COMMON_SYMBOLS = 150

# Максимум монет, для которых тянем свечи (pre-фильтр по 24h change)
MAX_CANDLE_CANDIDATES = 60

# Минимальное движение 24h, чтобы вообще смотреть монету
...
log = logging.getLogger("CEX-AGGREGATOR-v4.4")
...
async def fetch_bitget_tickers():
    ...
    for row in data.get("data", []):
        ...
        change24 = num(row.get("change24h") or row.get("changeUtc24h"))
        result[base] = {
            "symbol": symbol,
            "price": price,
            "volume24": quote_vol,
...
async def fetch_perpfinder_oi(base):
    """
    PerpFinder open-interest API.
    Документация: https://perpfinder.com (Quick start: fetch('/api/data/...'))
    Endpoint: /api/data/open-interest?asset=X&venue=Y
    """
    asset = base.upper()
    ...
    # Первые 500 символов сырого ответа
    try:
    ...
    # Вариант 2: {"data": [...]}
        ...
        rows = data["data"]
    # Вариант 3: [...] — массив прямо в корне
    elif isinstance(data, list):
        rows = data

    if rows is None:
        log.warning("[PerpFinder][%s] no rows field, top keys: %s",
                    asset, list(data.keys()) if isinstance(data, dict) else "list")
        STATS["rejected_perpfinder_empty"] += 1
        return None

    # Суммируем OI по всем строкам
    for row in rows:
        ...
        # Если OI в контрактах — умножаем на цену для приведения к USD
        if oi_val > 0:
            price = num(row.get("price") or row.get("mark_price") or 0.0)
            if price > 0:
                # Считаем, что это USD-значение если велико, иначе контракты
                if oi_val > 1_000_000:
                    total_oi_usd += oi_val
                else:
    ...
    log.info("[PerpFinder][%s] rows=%d, total_oi_usd=%.2f",
...
async def process_symbol(base):
    ...
    log.info("SHELF candidate: %s | breakout=+%.2f%% | fetching PerpFinder...",
    ...
    if oi["delta_pct"] < MARKET_MIN_OI_GROWTH_PCT:
        ...
        log.info("REJECT %s: OI delta %.2f%% < %.2f%%",
...
async def refresh_universe():
    ...
    for base, b in bitget.items():
        ...
        # Pre-фильтр по 24h change — отсеиваем «спящие» монеты
        change_abs = abs(b.get("change24") or 0)
        if change_abs < MIN_24H_CHANGE_PCT:
            continue
    ...
    log.info(
        "UNIVERSE | Bitget total=%d | in price+vol+change=%d | Common(top %d)=%d",
        len(bitget), len(common), MAX_COMMON_SYMBOLS, len(COMMON_SYMBOLS)
    )


def select_candle_candidates():
    """Топ-N монет по |24h change| — только для них тянем свечи."""
...
async def scan_cycle():
    ...
    # Семафор 3 — быстрее и всё ещё безопасно для Bitget
...
async def start_background(app):
    ...
    await send_telegram(
        "🚀 <b>CEX AGGREGATOR v4.4 ЗАПУЩЕН</b>\n"
