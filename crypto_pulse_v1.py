...
# ============================================================
#       CEX FUTURES AGGREGATOR v2.2 (FIXED BITGET OI ENGINE)
# ============================================================
...
async def fetch_bitget_oi(symbol):
    # Убеждаемся, что символ корректного формата Bitget
    if not symbol.endswith("USDT"):
        symbol = f"{symbol}USDT"

    # 1. Запрос к роуту open-interest
    ...
    # 2. ФОЛЛБЭК: Ищем значение напрямую из тикера BITGET_DATA
    base = normalize_base(symbol)
...
async def start_background(app):
    ...
    startup_msg = (
        "🚀 <b>CEX FUTURES AGGREGATOR v2.2 ЗАПУЩЕН</b>\n\n"
        "• Биржи: Bitget + KuCoin + Binance(vision)\n"
        f"• Ценовой фильтр: {MIN_PRICE_USDT}–{MAX_PRICE_USDT} USDT\n"
        "• Агрегация объема: пересчет в $\n"
        "• OI Bitget: с поддержкой прямого эндпоинта и тикеров\n"
        "• Статус: поиск аномалий запущен."
    )
    asyncio.create_task(send_telegram(startup_msg))

    log.info("CEX FUTURES AGGREGATOR v2.2 STARTED")
