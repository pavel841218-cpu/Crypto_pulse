...
MIN_BREAKOUT_PCT = 0.8            # Сигнал от +0.8% пробоя
MAX_BREAKOUT_PCT = 15.0           # Макс. +15% от верха полки
...
ALERT_COOLDOWN_SECONDS = 14400    # Кулдаун 4 часа
...
async def analyze_4h_setup(session, symbol, current_price):
    ...
    try:
        async with session.get(url, params=params, timeout=8) as resp:
            ...
            # 4. Расчет экстраполированного RVOL
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        # Небольшая пауза для гарантированного подключения сети
