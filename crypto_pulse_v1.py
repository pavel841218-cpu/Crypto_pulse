...
# Наш URL на Render для защиты от спячки
...
# ==========================================
# РЫНОЧНЫЕ ДАННЫЕ, МОНИТОРИНГ И САМОПИНГ
# ==========================================
async def keep_alive_ping():
    """Фоновая задача самопинга для обхода ограничений Free Render"""
    await asyncio.sleep(30)  # Даем веб-серверу время на запуск
    logging.info("Анти-сон система (Самопинг) успешно активирована.")
    while True:
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.get(SELF_URL)
                if response.status_code == 200:
                    logging.info("Самопинг: успешно отправил сигнал бодрствования на Render.")
                else:
                    logging.warning(f"Самопинг вернул статус: {response.status_code}")
        except Exception as ping_err:
            logging.error(f"Ошибка выполнения самопинга: {ping_err}")
        # Пингуем каждые 4 минуты (240 секунд), так как Render тушит инстансы через 5 минут простоя
...
async def main():
    ...
    asyncio.create_task(keep_alive_ping())  # <--- Запускаем анти-сон таск тут!
    
    # 4. Запуск поллинга напрямую в текущем цикле
    try:
