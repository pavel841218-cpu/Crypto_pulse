...
MAX_CANDLE_CANDIDATES = 120  # было 60

MIN_24H_CHANGE_PCT = 2.0  # мягче, чем 3.0
...
# Мягче импульс
BITGET_MIN_RVOL = 1.8       # было 2.0
BITGET_MIN_MOVE_PCT = 0.5   # было 0.7

# Полка короче и шире
CONSOLIDATION_HOURS = 6     # было 12
MAX_CONSOLIDATION_RANGE_PCT = 5.0  # было 3.2

MARKET_MIN_OI_GROWTH_PCT = 0.15

MIN_CANDLES = 144           # 12h по 5m (было 288)
            ...
            if resp.status == 200:
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        log.info("List length: %d", len(data))
                        if data:
                            log.info("First row keys: %s", list(data[0].keys()) if isinstance(data[0], dict) else "n/a")
                    return "OK"
                except Exception as e:
...
async def start_background(app):
    ...
    # Тест PerpFinder ДО запуска сканера
