...
# ============================================================
#       CEX FUTURES AGGREGATOR v2.5 (COINGLASS HISTORY CHART)
# ============================================================
...
# ============================================================
# COINGLASS (exchange-history-chart)
# ============================================================
...
async def fetch_binance_coinglass(base):
    ...
    # Рабочий V4 endpoint для графика/истории OI по бирже
    url = f"{COINGLASS_BASE}/api/futures/open-interest/exchange-history-chart"
    headers = {
        "CG-API-KEY": COINGLASS_API_KEY,
        "accept": "application/json",
    }

    # В V4 параметр 'symbol' передаёт чистый тикер монеты (например, "BTC", "BR")
    params = {
        "symbol": base.upper(),
        "exchange": "Binance",
        "range": "1h",  # Допустимые значения: 1h, 4h, 12h, all
    }
    ...
    if isinstance(raw_data, dict):
        data_map = raw_data.get("data_map", {})
        binance_oi_list = data_map.get("Binance") or data_map.get("binance")

        if binance_oi_list and isinstance(binance_oi_list, list):
            last_oi = num(binance_oi_list[-1])
            if last_oi > 0:
                STATS["cg_success"] += 1
                return {"exchange": "Binance", "oi": last_oi}
...
async def send_flow_signal(base, flow, oi, binance_oi, binance_price=None):
    ...
    message = (
        ...
        f"RVOL: {k['rvol']:.2f}x\n"
        f"Объём 5m ($): ${k['volume_usd']:,.0f}\n"
        f"Сила закрытия: {k['close_position']:.2f}\n\n"
        "━━━━━━━━━━━━━━\n"
        "🌐 <b>АГРЕГИРОВАННЫЙ ПОТОК ФЬЮЧЕРСОВ</b>\n"
        f"Суммарный объём 5m ($): ${(b['volume_usd'] + k['volume_usd']):,.0f}\n"
...
async def hourly_diagnostics():
    ...
    log.info(
        ...
        "OI: api=%d ticker=%d miss=%d | CG: calls=%d ok=%d empty=%d err=%d",
        len(bitget), len(kucoin), len(binance),
        ...
        STATS["bg_oi_missing"],
        STATS["cg_calls"], STATS["cg_success"],
        STATS["cg_empty"], STATS["cg_errors"],
    )
    ...
    msg = (
        ...
        f"<b>CoinGlass</b>\n"
        f"calls: {STATS['cg_calls']} | ok: {STATS['cg_success']} | "
        f"empty: {STATS['cg_empty']} | errors: {STATS['cg_errors']}\n\n"
...
COINGLASS STATS
---------------
Calls:   {STATS["cg_calls"]}
Success: {STATS["cg_success"]}
Empty:   {STATS["cg_empty"]}
Errors:  {STATS["cg_errors"]}
...
async def start_background(app):
    ...
    # --- ТЕСТОВЫЙ ЗАПРОС К COINGLASS (exchange-history-chart) ---
    cg_status = "❌ Не настроен (нет ключа)"

    if COINGLASS_API_KEY:
        log.info("=" * 60)
        log.info("COINGLASS API TEST START (exchange-history-chart)")
        log.info("Key length: %d", len(COINGLASS_API_KEY))
        log.info("-" * 60)
        ...
        try:
            async with SESSION.get(
                ...
                text = await resp.text()
                log.info("Response (first 700 chars): %s", text[:700])

                if resp.status == 200:
                    try:
                        data = await resp.json()
                        log.info("JSON code: %s", data.get("code"))
                        log.info("JSON msg: %s", data.get("msg"))

                        raw = data.get("data")
                        if isinstance(raw, dict):
                            data_map = raw.get("data_map", {})
                            binance_oi_list = (
                                data_map.get("Binance")
                                or data_map.get("binance")
                            )
                            if binance_oi_list and isinstance(binance_oi_list, list):
                                log.info(
                                    "Binance OI points: %d",
                                ...
                                last_oi = num(binance_oi_list[-1])
                                if last_oi > 0:
                                    cg_status = f"✅ OK (BTC OI Binance: {last_oi:,.0f})"
                                else:
                                    cg_status = "⚠️ Список есть, но OI=0"
                            else:
                                cg_status = "⚠️ В ответе нет Binance OI"
                                log.warning("data_map keys: %s", list(data_map.keys()))
                        else:
                            cg_status = f"⚠️ Пусто. code={data.get('code')}"
                            log.warning("No data in response")
                    except Exception as e:
                        cg_status = f"⚠️ Ошибка парсинга JSON: {e}"
                        log.warning("JSON parse error: %s", e)
                elif resp.status == 401:
                    cg_status = "❌ Ошибка 401: неверный API ключ"
                elif resp.status == 403:
                    cg_status = "❌ Ошибка 403: нет доступа к эндпоинту"
                elif resp.status == 429:
                    cg_status = "❌ Ошибка 429: превышен лимит"
                else:
                    cg_status = f"❌ HTTP {resp.status}"

        except asyncio.TimeoutError:
            cg_status = "❌ Таймаут запроса к CoinGlass"
        except Exception as e:
            cg_status = f"❌ Ошибка запроса: {e}"
            log.error("CoinGlass test error: %s", e)

        log.info("-" * 60)
        log.info("COINGLASS RESULT: %s", cg_status)
        log.info("=" * 60)
    else:
        log.warning("COINGLASS_API_KEY NOT SET — Binance OI via CoinGlass disabled")
    # -----------------------------------

    startup_msg = (
        "🚀 <b>CEX FUTURES AGGREGATOR v2.5 ЗАПУЩЕН</b>\n\n"
        "• Биржи: Bitget + KuCoin + Binance(vision)\n"
        f"• CoinGlass: {cg_status}\n"
    ...
    log.info("CEX FUTURES AGGREGATOR v2.5 STARTED")
