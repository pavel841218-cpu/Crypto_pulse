...
# ============================================================
#       CEX FUTURES AGGREGATOR v2.3 (COINGLASS TEST)
# ============================================================
...
async def start_background(app):
    ...
    # --- ТЕСТОВЫЙ ЗАПРОС К COINGLASS ---
    cg_status = "❌ Не настроен (нет ключа)"

    if COINGLASS_API_KEY:
        log.info("=" * 60)
        log.info("COINGLASS API TEST START")
                ...
                if resp.status == 200:
                    try:
                        ...
                        if isinstance(rows, list) and rows:
                            log.info("Rows count: %d", len(rows))
                            for r in rows[:5]:
                                ex = r.get("exchange") or r.get("exchangeName") or "?"
                                oi = (
                                    r.get("openInterest")
                                    or r.get("oi")
                                    or r.get("open_interest")
                                    or 0
                                )
                                log.info("  %s → OI=%s", ex, oi)

                            binance_oi = None
                            for r in rows:
                                ex = str(
                                    r.get("exchange")
                                    or r.get("exchangeName")
                                    or ""
                                ).lower()
                                if ex == "binance":
                                    binance_oi = num(
                            ...
                            if binance_oi and binance_oi > 0:
                                cg_status = f"✅ OK (BTC OI Binance: {binance_oi:,.0f})"
                            else:
                                cg_status = "⚠️ API отвечает, но Binance OI не найден"
                        else:
                            cg_status = "⚠️ API отвечает, но данных нет"
                    ...
                    cg_status = "❌ Ошибка 403: нет доступа к эндпоинту (тариф?)"
    ...
    startup_msg = (
        "🚀 <b>CEX FUTURES AGGREGATOR v2.3 ЗАПУЩЕН</b>\n\n"
    ...
    log.info("CEX FUTURES AGGREGATOR v2.3 STARTED")
