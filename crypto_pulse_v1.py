...
async def fetch_bitget_oi(symbol):
    ...
    row = rows[0]
    return num(row.get("openInterest") or row.get("oi") or row.get("openInterestUsd"))
...
async def send_ignition_signal(base, ig):
    message = (
        ...
        f"Move (forming 5m): {ig['bitget_move']:+.2f}% / {ig['kucoin_move']:+.2f}%\n"
        f"Forming RVOL: {ig['bitget_forming_rvol']:.2f}x / {ig['kucoin_forming_rvol']:.2f}x\n\n"
...
async def send_flow_signal(base, flow, oi, binance_oi, binance_price=None):
    ...
    if binance_price:
        message += f"💵 Binance цена: {binance_price:.6f}\n\n"

    message += "⚠️ Это ранний поток, а не сигнал входа.\nОжидаем продолжение / ретест."
    await send_telegram(message)
...
async def hourly_diagnostics():
    """
    Раз в час: живы ли биржи, что реально приходит
    (тикеры / объём / OI / RVOL по sample).
    Обычные scan-логи не трогаем.
    """
    ...
    log.info(
        ...
        "req BG=%d/%d KC=%d/%d BN=%d/%d CG=%d/%d",
        len(bitget), len(kucoin), len(binance),
    ...
    msg = (
        "🩺 <b>HOURLY DIAG</b>\n\n"
        f"<b>Tickers</b>\n"
        f"Bitget: {len(bitget)} | vol24≈${vol_sum(bitget)/1e6:.1f}M\n"
        f"KuCoin: {len(kucoin)} | vol24≈${vol_sum(kucoin)/1e6:.1f}M | OI sum≈{oi_sum_kucoin(kucoin):,.0f}\n"
        f"Binance vision: {len(binance)} | vol24≈${vol_sum(binance)/1e6:.1f}M\n\n"
        f"<b>Common</b>: {len(COMMON_SYMBOLS)}\n"
        f"<b>Sample</b>: {sample_base or 'n/a'}\n"
        f"Bitget candles: {len(bg_candles)} | RVOL≈{bg_rvol:.2f}x | OI={bg_oi:,.0f}\n"
        f"KuCoin candles: {len(kc_candles)} | RVOL≈{kc_rvol:.2f}x | OI={kc_oi:,.0f}\n\n"
        f"<b>HTTP</b>\n"
        f"BG {STATS['bitget_requests']}/{STATS['bitget_errors']} err | "
        f"KC {STATS['kucoin_requests']}/{STATS['kucoin_errors']} err | "
        f"BN {STATS['binance_requests']}/{STATS['binance_errors']} err | "
        f"CG {STATS['coinglass_requests']}/{STATS['coinglass_errors']} err\n\n"
        f"Signals: ign={STATS['ignition_signals']} "
...
async def scanner_loop():
    ...
    while True:
        try:
            ...
            # часовая диагностика — отдельно от scan-логов
...
async def start_background(app):
    ...
    startup_msg = (
        ...
        "• Hourly diag: тикеры / RVOL / OI раз в час\n"
