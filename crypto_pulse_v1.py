...
# ============================================================
#       CEX FUTURES AGGREGATOR v2.1 (FIXED)
#
...
async def fetch_bitget_oi(symbol):
    ...
    # ВРЕМЕННЫЙ ОТЛАДОЧНЫЙ ЛОГ ДЛЯ ДИАГНОСТИКИ (ARB / SAMPLE)
    if "ARB" in symbol:
        log.info("RAW BITGET OI RESPONSE FOR %s: %s", symbol, data)
    ...
    # ФИКС ПАРСИНГА BITGET OI: обработка всех вариантов структурирования V2 API
...
# ============================================================
# MESSAGES (РУСИФИЦИРОВАНЫ + КОПИРУЕМЫЕ ТИКЕРЫ)
# ============================================================
...
async def process_symbol(base):
    ...
    # ФИКС ЛОГИКИ ФИЛЬТРАЦИИ OI ДЛЯ SHORT
    if flow["direction"] == "LONG":
        if oi["avg_delta"] < MIN_OI_GROWTH_PCT:
            STATS["rejected_oi"] += 1
            return
    else:  # SHORT
        if oi["avg_delta"] < MIN_OI_GROWTH_PCT: # Позиции наращиваются и на SHORT!
            STATS["rejected_oi"] += 1
            return
