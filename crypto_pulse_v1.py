...
# Проверяем наличие актуального ОИ, но НЕ ждем 10 минут накопительной истории
...
# Желаемое окно дельты ОИ (до 10 минут)
OI_DELTA_LOOKBACK_SEC = 600

# Максимальный возраст последнего OI (секунды)
...
async def send_startup_message(session):
    message = (
        ...
        f"</b>\n"
        f"🎯 Фильтр ОИ: <b>{'ВКЛ (Мгновенные сигналы)' if REQUIRE_VALID_OI else 'ВЫКЛ'}</b>\n"
        f"🔥 Short RVOL: <b>5m</b>\n\n"
        "🚀 <i>Мониторинг рынка начат. Режим мгновенной отправки активен!</i>"
    )
...
# ============================================================
#              OI HISTORY — ЛОГИКА БЕЗ ЗАДЕРЖЕК
# ============================================================

def store_oi_sample(symbol, current_oi, now_timestamp):
    ...
    if history:
        ...
        if (now_timestamp - last_time) < 1:
            return

    history.append((now_timestamp, current_oi))
    CURRENT_OI[symbol] = {"time": now_timestamp, "oi": current_oi}


def get_oi_change_and_time(symbol, now_timestamp):
    """
    Возвращает процент изменения ОИ и количество секунд, за которые было измерено
    """
    history = OI_HISTORY.get(symbol)
    if not history:
        return None, 0
    ...
    if (now_timestamp - latest_time) > MAX_OI_STALENESS_SEC:
        return None, 0

    # Если в истории всего 1 запись — процент изменения 0.0%
    if len(history) == 1:
        return 0.0, 0

    # Ищем самую старую точку в пределах 10 минут (или самую давнюю из доступных)
    ...
    change_pct = ((current_oi - old_oi) / old_oi) * 100.0
    elapsed_sec = int(latest_time - old_time)
    
    return round(change_pct, 2), elapsed_sec


def has_valid_current_oi(symbol, now_timestamp):
    """Проверяет просто наличие любого свежего ОИ по монете"""
    curr = CURRENT_OI.get(symbol)
    if not curr:
        return False
    if (now_timestamp - curr["time"]) > MAX_OI_STALENESS_SEC:
        return False
    return curr["oi"] > 0
...
async def collect_oi_for_all(session, tickers):
    if not tickers:
        return 0

    now_timestamp = time.time()
    successful = 0
    symbols = [item[0] for item in tickers]
    
    for i in range(0, len(symbols), OI_BATCH_SIZE):
        batch = symbols[i:i + OI_BATCH_SIZE]
        tasks = [fetch_open_interest(session, sym) for sym in batch]
        
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        for sym, res in zip(batch, results):
            if isinstance(res, (int, float)) and res > 0:
                store_oi_sample(sym, res, now_timestamp)
                successful += 1
                
        await asyncio.sleep(OI_BATCH_DELAY)
...
async def process_tickers(session, tickers):
    ...
    for (symbol, current_price, quote_volume) in tickers:
        ...
        # Проверяем просто актуальность ОИ (чтобы не шло спотовых монет без ОИ)
        if REQUIRE_VALID_OI:
            if not has_valid_current_oi(symbol, now_timestamp):
                waiting_oi += 1
                logging.info("⏳ ОИ не получен | %s | цена %.2f%%", symbol, change_pct)
                continue

        oi_change_pct, elapsed_sec = get_oi_change_and_time(symbol, now_timestamp)
        ...
        # Красиво форматируем время накопившегося ОИ
        oi_time_str = f"{elapsed_sec // 60}м" if elapsed_sec >= 60 else f"{elapsed_sec}с"
        oi_text = f"{oi_emoji} {oi_change_pct:+.2f}% (за {oi_time_str})" if elapsed_sec > 0 else f"{oi_emoji} 0.00% (новое)"
        ...
        message = (
            ...
            f"{rvol_comment}\n\n"
            f"👁 <b>Открытый интерес:</b> {oi_text}\n"
        ...
        alerts += 1
        logging.info(
            "🚀 СИГНАЛ | %s | %s%.2f%% за %dm | RVOL 1H %.2fx | Short RVOL %.2fx | OI %+.2f%% (%s)",
            clean_coin, sign, change_pct, actual_minutes, hourly_rvol, short_rvol, oi_change_pct, oi_time_str
        )

    return ready, alerts, waiting_oi
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        logging.info("🚀 CRYPTO_PULSE ЗАПУЩЕН")
        ...
        while True:
            ...
            if tickers:
                ...
                oi_success = await collect_oi_for_all(session, tickers)
                ready, alerts, waiting_oi = await process_tickers(session, tickers)

                logging.info(
                    "📊 СКАН | пар=%d | готово=%d | OI получено=%d | ожидают OI=%d | сигналов=%d | история цены=%d | история OI=%d",
                    len(tickers), ready, oi_success, waiting_oi, alerts, len(PRICE_HISTORY), len(OI_HISTORY)
                )
            else:
