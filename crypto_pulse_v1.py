...
# ------------------------------------------------------------
# NEW: ФИЛЬТР ПО ЦЕНЕ МОНЕТЫ
# Ищем монеты в диапазоне от 0.001 до 1 USDT.
# Дешёвые монеты дают больший % памп.
# ------------------------------------------------------------
...
STATS = {
    ...
    # NEW: статистика фильтра по цене
    "price_in_range": 0,
...
def build_common_universe(bitget, kucoin, binance):
    ...
    NEW: фильтр по цене 0.001 <= price <= 1 USDT.
    """
...
async def refresh_universe():
    ...
    # NEW: подсчёт монет в ценовом диапазоне
    in_range = 0
...
def select_candle_candidates():
    """
    Кандидаты для свечей — фильтр по цене уже применён в universe.
    """
