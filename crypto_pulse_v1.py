...
SHELF_MIN_HOURS = 6
SHELF_MAX_WIDTH_PCT = 5.0
EMA_COMPRESSION_MAX_PCT = 2.5
...
async def health_check(request):
    return web.Response(text="Shelf Breakout Hunter is Live!", status=200)
...
async def get_multi_oi(session, symbol, semaphore):
    ...
    if not valid:
        return {"bybit": None, "bingx": None, "consensus": 0.0,
                "divergence": False, "spread": 0.0, "sources_count": 0,
    ...
    return {
        ...
        "consensus": consensus, "divergence": divergence, "spread": spread,
        "sources_count": len(valid), "positive_count": positive,
...
def analyze_1h_context(candles_1h):
    ...
    ema_fast_list = calculate_ema(closes, EMA_1H_FAST)
    ema_mid_list = calculate_ema(closes, EMA_1H_MID)
    ema_slow_list = calculate_ema(closes, EMA_1H_SLOW)

    if not ema_fast_list or not ema_mid_list or not ema_slow_list:
        return None

    ema_fast = ema_fast_list[-1]
    ema_mid = ema_mid_list[-1]
    ema_slow = ema_slow_list[-1]

    # Все EMA внутри полки
    for e in (ema_fast, ema_mid, ema_slow):
        if not (base_low <= e <= base_high):
            return None

    # Сжатие между EMA
    ema_vals = [ema_fast, ema_mid, ema_slow]
    ...
    return {
        ...
        "ema20": ema_fast, "ema40": ema_mid, "ema80": ema_slow,
...
def detect_impulse_15m(candles_15m, shelf_ctx):
    ...
    return {
        ...
        "stop_loss": stop_loss, "risk_pct": risk_pct,
        "tp1": tp1, "tp2": tp2,
    }
...
async def send_signal(bot, symbol, sig, oi_data):
    try:
        ...
        if USE_OI_FILTER and oi_data:
            ...
            if oi_data.get("divergence"):
                oi_lines += "⚠️ <b>ДИВЕРГЕНЦИЯ OI</b>\n"

        warning = ""
        if sig.get("oi_warning"):
            warning = "⚠️ <b>ОСТОРОЖНО: стоп-хант</b>\n\n"

        message = (
            f"🚀 <b>ПРОБОЙ ПОЛКИ (EMA 20/40/80)</b>\n\n"
            f"{warning}"
            f"🪙 Монета: <code>{coin}</code>\n\n"
            f"💥 Пробой: <b>+{sig['breakout_pct']:.2f}%</b>\n"
            ...
            f"{oi_lines}\n"
            f"📦 <b>ПОЛКА 1H:</b>\n"
            f"├ Ширина: <b>{sig['shelf_width_pct']}%</b>\n"
            ...
            f"💰 <b>ВХОД:</b> <code>{format_price(sig['current_price'])}</code>\n"
            f"🛑 <b>Стоп:</b> <code>{format_price(sig['stop_loss'])}</code> "
            f"(риск {sig['risk_pct']:.2f}%)\n"
            f"🎯 <b>TP1:</b> <code>{format_price(sig['tp1'])}</code> "
            f"(+{sig['risk_pct'] * TP1_RR:.2f}%)\n"
            f"🎯 <b>TP2:</b> <code>{format_price(sig['tp2'])}</code> "
...
async def check_symbol(session, bot, symbol, shelf_ctx, semaphore):
    ...
    if USE_OI_FILTER:
        ...
        if oi_data["divergence"]:
            if (bybit_ok and sig["volume_mult"] >= DIVERGENCE_STRONG_VOLUME
    ...
    success = await send_signal(bot, symbol, sig, oi_data)
    if success:
        last_signals[symbol] = now
        signals_this_hour.append(now)
        shelf_cache.pop(symbol, None)
        logging.info(
            f"🚀 СИГНАЛ {symbol} | пробой +{sig['breakout_pct']:.2f}% | "
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        try:
            await bot.send_message(
                chat_id=CHAT_ID,
                text=(
                    "🚀 <b>SHELF BREAKOUT HUNTER запущен</b>\n\n"
        ...
        while True:
            try:
                ...
                tasks = [check_symbol(session, bot, sym, ctx, semaphore)
                ...
                if impulse_scan_counter % 20 == 0 or signals > 0:
                    logging.info(
                        ...
                        f"Полок: {len(shelf_cache)} | Сигналов: {signals}"
                    )

                # Отчёт по отсевам раз в 40 сканов
