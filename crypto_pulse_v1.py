...
# ============================================================
#       CEX FUTURES AGGREGATOR v2.4 (COINGLASS CHART ENDPOINT)
# ============================================================
...
# ============================================================
# COINGLASS (НОВЫЙ ЭНДПОИНТ: chart)
# ============================================================
...
async def fetch_binance_coinglass(base):
    ...
    # Используем базовый эндпоинт, входящий в бесплатный/стандартный план
    url = f"{COINGLASS_BASE}/api/futures/openInterest/chart"
    headers = {"CG-API-KEY": COINGLASS_API_KEY, "accept": "application/json"}

    # Склеиваем в стандартную пару для Binance (например, BTCUSDT)
    symbol = f"{base.upper()}USDT"
    params = {
        "symbol": symbol,
        "exchange": "Binance",
        "interval": "0",  # 0 — забор текущего/последнего значения
    }
    ...
    if isinstance(raw_data, list) and raw_data:
        ...
        oi_val = num(
            last_entry.get("openInterest")
            or last_entry.get("oi")
            or last_entry.get("c")
        )
        if oi_val > 0:
            STATS["cg_success"] += 1
            return {"exchange": "Binance", "oi": oi_val}

    elif isinstance(raw_data, dict):
        oi_val = num(raw_data.get("openInterest") or raw_data.get("oi"))
        if oi_val > 0:
            STATS["cg_success"] += 1
            return {"exchange": "Binance", "oi": oi_val}
...
async def start_background(app):
    ...
    # --- ТЕСТОВЫЙ ЗАПРОС К COINGLASS (НОВЫЙ ЭНДПОИНТ) ---
    cg_status = "❌ Не настроен (нет ключа)"

    if COINGLASS_API_KEY:
        log.info("=" * 60)
        log.info("COINGLASS API TEST START (chart endpoint)")
        ...
        try:
            async with SESSION.get(
                ...
                text = await resp.text()
                log.info("Response (first 500 chars): %s", text[:500])
                ...
                if resp.status == 200:
                    try:
                        ...
                        if isinstance(raw, list) and raw:
                            log.info("Data rows: %d", len(raw))
                            last = raw[-1]
                            log.info("Last entry: %s", last)
                            oi_val = num(
                            ...
                            if oi_val > 0:
                                cg_status = f"✅ OK (BTC OI Binance: {oi_val:,.0f})"
                            else:
                                cg_status = "⚠️ Данные есть, но OI=0"
                        elif isinstance(raw, dict):
                            log.info("Data dict: %s", raw)
                            oi_val = num(raw.get("openInterest") or raw.get("oi"))
                            if oi_val > 0:
                                cg_status = f"✅ OK (BTC OI Binance: {oi_val:,.0f})"
                            else:
                                cg_status = "⚠️ Данные есть, но OI=0"
                        else:
                            cg_status = f"⚠️ Пусто. code={data.get('code')} msg={data.get('msg')}"
                            log.warning("No data rows")
                    except Exception as e:
                    ...
                    cg_status = "❌ Ошибка 429: превышен лимит запросов"
                else:
    ...
    startup_msg = (
        "🚀 <b>CEX FUTURES AGGREGATOR v2.4 ЗАПУЩЕН</b>\n\n"
    ...
    log.info("CEX FUTURES AGGREGATOR v2.4 STARTED")
