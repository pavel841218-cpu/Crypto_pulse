...
# ============================================================
#       CEX FUTURES AGGREGATOR v2.6 (DIRECT BINANCE OI)
# ============================================================
...
# ============================================================
# BINANCE FUTURES — прямой OI (замена CoinGlass)
# ============================================================

async def fetch_binance_oi_direct(base):
    """
    Прямой запрос Open Interest с Binance Futures API.
    Публичный эндпоинт, не требует API ключа.
    """
...
# ============================================================
# COINGLASS (оставлено для обратной совместимости, но не используется)
# ============================================================
...
async def process_symbol(base):
    ...
    binance_oi = await fetch_binance_oi_direct(base)
    if binance_oi:
...
async def start_background(app):
    ...
    # --- ТЕСТ ПРЯМОГО BINANCE OI ---
    bn_status = "⚠️ не проверено"

    log.info("=" * 60)
    log.info("BINANCE FUTURES OI TEST START")
    log.info("-" * 60)
    ...
    try:
        ...
        async with SESSION.get(
            ...
            text = await resp.text()
            log.info("Response: %s", text[:300])

            if resp.status == 200:
                data = await resp.json()
                oi = num(data.get("openInterest"))
                if oi > 0:
                    bn_status = f"✅ OK (BTC OI: {oi:,.0f})"
                else:
                    bn_status = "⚠️ OI=0"
            elif resp.status == 451:
                bn_status = "❌ 451: гео-блок (Render Frankfurt)"
            elif resp.status == 403:
                bn_status = "❌ 403: доступ запрещён"
            else:
                bn_status = f"❌ HTTP {resp.status}"

    except Exception as e:
        bn_status = f"❌ {e}"
        log.error("Binance OI test error: %s", e)
    ...
    startup_msg = (
        "🚀 <b>CEX FUTURES AGGREGATOR v2.6 ЗАПУЩЕН</b>\n\n"
        "• Биржи: Bitget + KuCoin + Binance(vision)\n"
        f"• Binance OI (direct): {bn_status}\n"
    ...
    log.info("CEX FUTURES AGGREGATOR v2.6 STARTED")
