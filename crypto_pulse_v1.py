...
# Ограничиваем параллельные запросы OI, чтобы BingX/Render не лимитировали IP
OI_MAX_CONCURRENT_REQUESTS = 8
...
async def send_startup_message(session):
    message = (
        ...
        f"🔥 Short RVOL: <b>5m</b>\n"
        f"👁 OI: <b>История за ~10 минут</b>\n\n"
        "🚀 <i>Мониторинг рынка начат.</i>\n"
        "⏳ <i>OI-история накапливается. Сигналы без готовой OI-дельты не отправляются.</i>"
    )
...
# ============================================================
#                  OPEN INTEREST (ОБНОВЛЕННЫЙ ПАРСЕР)
# ============================================================
...
def get_oi_change_pct(symbol, now_timestamp):
    history = OI_HISTORY.get(symbol)
...
def has_ready_oi_history(symbol, now_timestamp):
    history = OI_HISTORY.get(symbol)
    if not history or len(history) < 2:
        return False
...
async def collect_oi_for_symbol(session, symbol, semaphore, now_timestamp):
    async with semaphore:
        oi = await fetch_open_interest(session, symbol)

    if oi is not None and oi > 0:
        store_oi_sample(symbol, oi, now_timestamp)
        return True
...
async def collect_oi_for_all(session, tickers):
    ...
    semaphore = asyncio.Semaphore(OI_MAX_CONCURRENT_REQUESTS)

    tasks = [
        collect_oi_for_symbol(session, symbol, semaphore, now_timestamp)
        for (symbol, _price, _volume) in tickers
    ]
...
async def process_tickers(session, tickers):
    ...
    for (symbol, current_price, quote_volume) in tickers:
        ...
        if REQUIRE_VALID_OI:
            if not has_ready_oi_history(symbol, now_timestamp):
                waiting_oi += 1
                logging.info("⏳ OI ещё не готов | %s | цена %.2f%%", symbol, change_pct)
                continue

        oi_change_pct = get_oi_change_pct(symbol, now_timestamp)
        if oi_change_pct is None:
            waiting_oi += 1
            logging.info("⏳ OI дельта не готова | %s", symbol)
            continue
        ...
        oi_text = f"{oi_emoji} {oi_change_pct:+.2f}% (за ~10м)"
        ...
        alerts += 1
        logging.info(
            "🚀 СИГНАЛ | %s | %s%.2f%% за %dm | RVOL 1H %.2fx | Short RVOL %.2fx | OI %+.2f%%",
            clean_coin, sign, change_pct, actual_minutes, hourly_rvol, short_rvol, oi_change_pct
        )
