...
def detect_order_block_and_bos(candles):
    ...
    for i in range(len(lows) - 1, 0, -1):
        ...
        # Ордер-блок = последняя медвежья свеча перед импульсом
        ob_candle = None
...
def check_retest(candles, setup):
    ...
    # Цена должна дотянуться до зоны
    in_zone = (
    ...
    # Сильное закрытие ниже зоны = сетап мёртв
...
async def check_symbol(session, bot, symbol, volume):
    ...
    if symbol in active_setups:
        ...
        if retest:
            ...
            # Если зона сильно пробита — удаляем сетап
            current = candles[-1]
