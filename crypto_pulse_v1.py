...
PORT = int(os.getenv("PORT", 8081))  # Отдельный порт для второго процесса
BINGX_BASE = "https://open-api.bingx.com"

MIN_24H_VOLUME = 1_000_000  # Фильтр ликвидности: от $1M
    ...
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            for item in data.get("data", []):
                ...
                # Исключаем индексы, активы фондового рынка, металлы и спец. тикеры
...
async def get_klines(session, symbol, interval, limit=50):
    ...
    try:
        async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            return candles[::-1]  # Сортировка от старых к новым
    except Exception as e:
        logging.error(f"Klines error {symbol} ({interval}): {e}")
        return []

# --- МОДУЛЬ ДЕТЕКЦИИ ПАМПА В НАЧАЛЕ (1M + 15M) ---
def analyze_fast_pump(candles_1m, candles_15m):
    # 1. Тренд на 15m (Бычий веер EMA7 > EMA25 > EMA50)
    ...
    # ТОЧКА ВХОДА В НАЧАЛЕ ДВИЖЕНИЯ:
    # - Аномальный RVOL 1m >= 3.0x
    # - Рост свечи строго от +0.2% до +0.8% (не на верхушке!)
    # - Текущее закрытие пробивает хай предыдущей минутки
    ...
    while True:
        try:
            ...
            for symbol in tickers:
                ...
                # Задержка 0.15с между парами для соблюдения Rate Limits API
        ...
        # Пауза 10 секунд перед следующим кругом сканирования рынка
...
async def cmd_start(msg: types.Message):
    ...
    await msg.answer("⚡️ Сканер 1m-пампов запущен! Мониторинг бычьих импульсов активен...")
