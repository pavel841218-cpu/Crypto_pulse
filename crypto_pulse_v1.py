...
# ИСПРАВЛЕНО: Увеличены лимиты, чтобы сканировать весь диапазон 300+ монет
MAX_COMMON_SYMBOLS = 350
MAX_CANDLE_CANDIDATES = 350
MAX_OI_CANDIDATES = 50
...
# ------------------------------------------------------------
# ФИЛЬТР ПО ЦЕНЕ МОНЕТЫ (0.001 - 1.0 USDT)
# ------------------------------------------------------------
...
def build_common_universe(bitget, kucoin, binance):
    """
    Общая вселенная — Bitget + KuCoin в диапазоне 0.001 <= price <= 1.0 USDT.
    ИСПРАВЛЕНО: Снято ограничение [:MAX_COMMON_SYMBOLS], отдаёт ВСЕ найденные монеты.
    """
    ...
    # Сортируем по объему, но возвращаем все в рамках лимита MAX_COMMON_SYMBOLS (350)
    sorted_common = dict(
        sorted(
            common.items(),
...
async def scan_cycle():
    ...
    # ИСПРАВЛЕНО: Добавлены Semaphore(5) и пачки с паузами против 429 ошибки Bitget
